...
for b_index, basis in enumerate(BASIS_SIZE):
	for iter_index, num_iter in enumerate(ITERATIONS):
		with open ("./../logs/{1}{0}cpu_timing_output.txt".format(b_index+1, iter_index+1), "r") as input_file:
...
for b_index, basis in enumerate(BASIS_SIZE):
	for iter_index, num_iter in enumerate(ITERATIONS):
		with open ("./../pycuda_logs/{2}/{0}_{1}base64_cpu.txt".format(basis, num_iter, "gpu003"), "r") as input_file:
			for line in input_file:
...
for b_index, basis in enumerate(BASIS_SIZE):
	for iter_index, num_iter in enumerate(ITERATIONS):
		with open ("./../pycuda_logs/{2}/{0}_{1}base64_cpu.txt".format(basis, num_iter, "kepler"), "r") as input_file:
			for line in input_file:
				if line.startswith("Time to run"):
...
# okay now we plot
# setup data structs
datasets = [io_003, gpu_003, kepler]
...
scaled_io, scaled_gpu, scaled_kepler = scaled#datasets
...
for i_index in range(kepler.shape[1]):
	...
	ax = figure_others.add_subplot(111)
	plt.subplots_adjust(left=0.0, right=1.0, bottom=0.0, top=1.0)
	rects1 = ax.bar(indicies, scaled_io[:, i_index], width_one, color='r')
	rects2 = ax.bar(indicies+width_one, scaled_gpu[:, i_index], width_one, color='y')
	rects3 = ax.bar(indicies+width_two, scaled_kepler[:, i_index], width_one, color='b') #color='k'
	autolabel(rects1)
	autolabel(rects2)
	autolabel(rects3)
	ax.set_ylabel('# times faster')
